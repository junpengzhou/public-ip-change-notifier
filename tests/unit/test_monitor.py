...
@pytest.mark.asyncio
async def test_monitor_when_known_ip_changes_then_notifies_and_saves_all_wans() -> None:
    service, store, notifier = make_service(
...
@pytest.mark.asyncio
async def test_monitor_when_multiple_wans_change_then_sends_one_notification() -> None:
    service, store, notifier = make_service(
        previous={"wan1": "203.0.113.10", "wan2": "198.51.100.20"},
